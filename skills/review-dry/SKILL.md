---
name: review-dry
description: Dry pull request review — the deliverable is a DRAFT comment, never a post. Check out the branch, review the diff through four lenses, run the util-qa skill for video evidence, and draft the comment from the canonical template (verdict lead, plain-prose Top points, 👓 QA section — video by default, frame-by-frame table on request). Posting, and the uploads it needs, happen only after the user reads the draft and explicitly asks. Use when asked to review a PR or branch.
---
...
FOR THESE REAL WORLD TESTS TO MAKE SENSE, OUR TESTS NEED TO BE FULLY THOUGHT OUT, THINKING END-TO-END FROM A CUSTOMER PERSPECTIVE.
...
Every finding has a provenance — you must always know internally whether it rests on a screenshot or a diff line, never blur the two — but the drafted comment renders every Top point as plain prose with no provenance or severity tag prefix. The distinction still drives ordering (severity, demonstrated-breakage-first):

- **Observed while running** — a defect you saw in a frame of the real run, each backed by that frame; empirical. Record these in `<qa-dir>/findings.json` (schema below); a failed checkpoint is an automatic blocker (already in `steps.json` — don't restate it). **Always relay the local `qa.mp4` path to the user in your chat reply** — the draft comment carries no video until something is uploaded, so the recording is only reachable if you surface its path.
- **From the diff** — findings from the four lenses, backed by `file:line`, not frames.
...
**Every drafted or posted comment uses this exact structure — no sections reordered, renamed, or silently omitted.** The visual spec is the approved mock: https://claude.ai/code/artifact/4bb8a30a-a135-449e-929c-eaaf8b63f3d8. Top points have no heading of their own — the numbered list sits directly under the lead — and each point is plain prose with no provenance or severity tag prefix; GitHub's sanitizer strips `class`/`style` anyway, so any such tag would render as inert text. The 👓 QA section's frame-by-frame table still has no color to lean on, so it reads by sorting the failing row first, opening its `<details>`, and carrying ⚠️.
...
1. <symptom observed on screen, naming its checkpoint; link the uploaded frame when posted>
2. <finding backed by `file:line`>
3. <...>
4. <the refactor suggestion, if it clears the 75/100 bar — always the lowest-priority point>
...
- **Top points** is the single merged, prioritized list — max four, ELI5, plain prose with no provenance or severity tag prefix. Internally, ordering still runs on severity — blocker | major | minor, never nit (nits are not reported anywhere) — and provenance: frame-backed observations (from `findings.json` and failed checkpoints) versus diff findings (with `file:line`). Every `findings.json` entry and every failed checkpoint must surface here; if more than four points survive triage, the four most severe go in, the tally says so, and the rest stay in the chat review — **but demonstrated breakage is never what gets cut.** Order and slot by this rule: a failed checkpoint or an on-screen defect (a *shown* break) outranks a same-severity finding argued only from the diff, and a broken behavior always keeps its slot over a cleanliness or theoretical point — trim those first. The lead names the breakage first. The refactor suggestion, when it clears the bar, takes the lowest-priority slot.
...
- **If QA was skipped** (the change has no observable surface, or the app genuinely could not boot — the only two sanctioned reasons), the `### 👓 QA` section instead contains one paragraph saying so and what you did instead. It is never silently omitted, and no QA-sourced point may appear in Top points.
