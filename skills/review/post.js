...
//   node post.js --login
//
// Upload one file (prints the bare asset URL):
//   node post.js <file> --repo owner/name --pr <number>
//
// Upload several (prints a JSON map of basename -> asset URL, reusing one
// browser session). --json forces the map form even for a single file:
//   node post.js <frame1> <frame2> … --repo owner/name --pr <number>
...
// Upload a single file into the already-open comment box and return its asset
// URL, clearing the draft afterwards so the next upload starts clean.
// @param {import('playwright').Page} page a PR page with the comment box loaded
// @param {string} file path to the file to upload
// @return {Promise<string>} the user-attachments asset URL
async function uploadFile(page, file) {
  ...
  // The page can hold several attachment inputs (e.g. the PR-body edit form's)
  // — target the one wired to the new-comment field specifically, or the upload
  // lands in the wrong form and the wait below never resolves.
...
// Upload one or more files, reusing a single logged-in browser session. Prints
// the bare URL for a lone file, or a JSON basename->URL map for several (or when
// --json is set) so a formatter can key frames back to their assets.
// @param {string[]} files paths to upload
// @param {string} repo owner/name
// @param {string|number} pr pull request number
// @param {boolean} asJson force the JSON-map output form
// @return {Promise<void>}
async function upload(files, repo, pr, asJson) {
  for (const file of files) if (!fs.existsSync(file)) throw new Error(`NO_FILE ${file}`);
  ...
  try {
    ...
    const map = {};
    for (const file of files) map[path.basename(file)] = await uploadFile(page, file);

    if (asJson || files.length > 1) console.log(JSON.stringify(map, null, 2));
    else console.log(map[path.basename(files[0])]);
  } finally {
    await ctx.close();
  }
}
...
async function main() {
  ...
  if (positionals.length < 1 || !values.repo || !values.pr) {
    console.error('Usage: post.js --login | post.js <file…> --repo owner/name --pr <number> [--json]');
    process.exit(1);
  }
  return upload(positionals, values.repo, values.pr, values.json);
}
