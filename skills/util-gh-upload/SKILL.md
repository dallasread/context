...
- Module resolution — this skill has no npm footprint; `post.js` resolves Playwright from the util-util-qa skill's install (which `qa.sh` self-heals on every run). If the resolve fails: `cd ~/.claude/skills/util-qa && npm install`.

## Callers

- **review-dry** — uploads the frames its `format.js --list-frames` names, plus `qa.mp4`, for the posted review comment.
