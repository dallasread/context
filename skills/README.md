...
| Skill | Hand it | You get back |
|---|---|---|
| **util-review** | a branch to review, and whether QA may run | the review itself: verdict, sections, findings anchored to lines, and the QA evidence. Owns the scenario authoring, the findings and how they are categorized. It formats nothing and publishes nothing. |
| **dev-review** | a review result and a drafts directory | that review written as a JSON draft the reviewer app reads. The other formatter, and the only place the draft file's shape lives. |
