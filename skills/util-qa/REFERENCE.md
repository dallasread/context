...
- `profiles/<repo>.js` — REQUIRED. A JS module exporting the machine config: boot + login. `chmod 600` it.
  ...
  `baseHost` is OPTIONAL: the hostname the app routes on when it isn't plain `localhost` (e.g. a subapp split serving the app only on `app.<name>.localhost`). qa.sh builds the run's base URL as `http://<baseHost>:$PORT` and everything downstream — readiness probe, browser, cookie host — follows; omit it and the base stays `http://localhost:$PORT`. Any `*.localhost` name resolves to loopback without `/etc/hosts`. Guarded by `test/base-host.test.sh`.
